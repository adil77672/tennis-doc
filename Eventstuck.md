...
1. **In the sync loop (markeUsersProcessing):** Before setting an event to InProgress, we skip it if both `sync_status` and `cron_status` are already Completed. So we never overwrite Completed → InProgress there.
2. **In processEventData (update path):** When updating an existing event on manual sync, we set `sync_status` to InProgress only if it is not already Completed; if it is Completed we leave it Completed.
...
1. Calls `markEventAsDone(liveEvent.event_id)` (memory lock).
2. Updates the event in the DB: `sync_status: Completed`, `cron_status: Completed`.
...
2. **Optional: cleanup job in backend**  
